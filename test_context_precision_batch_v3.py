"""
RAGAS Context Precision Test for Flynas Chatbot - V3 (Direct Response Waiting)
Tests the chatbot's context precision using the RAGAS framework with browser automation.
Waits for actual /chat API response instead of blind sleeping - eliminates race condition.

V3 Improvements:
- Direct response waiting using page.expect_response()
- No async handler race condition
- Single-threaded response processing
"""
...
class FlynasChatbotWithContextCapture:
    ...
    async def send_message(self, message: str) -> ChatbotResponse:
        ...
        try:
            ...
            # V3 KEY CHANGE: Use expect_response to wait for /chat API response directly
            # This eliminates the race condition from V2
            async with self.page.expect_response(
                lambda r: '/chat' in r.url and r.request.method == 'POST',
                timeout=60000  # 60 second timeout
            ) as response_info:
                # Click send (or press Enter) - this triggers the request
                if send_btn:
            ...
            # Get the response directly - no race condition!
            response = await response_info.value

            # Small delay to ensure response is fully received
            ...
            # Extract answer and contexts directly from the response
            answer, contexts = self._extract_contexts_from_response(body)

            # Additional wait for UI to update (for human-like behavior)
...
async def main():
    print("=" * 60)
    print("RAGAS Context Precision Test - Flynas Chatbot (V3)")
