"""
RAGAS Context Relevancy Test for Flynas Chatbot
Tests the chatbot's context relevancy using the RAGAS framework with browser automation.
Uses Direct Response Waiting (V3 pattern) - No Race Condition.

Context Relevancy measures whether the retrieved contexts are relevant to the question.
...
class ContextRelevancyEvaluator:
    """Evaluates chatbot responses using RAGAS Context Relevancy metric"""
    ...
    async def evaluate_single(self, question: str, answer: str, contexts: List[str]) -> dict:
        """Evaluate context relevancy for a single question"""
        try:
            if not contexts:
                return {
                    "question": question,
...
async def main():
    print("=" * 60)
    print("RAGAS Context Relevancy Test - Flynas Chatbot")
    ...
    RESULTS_DIR = "/home/shreyank06/Desktop/projects/testing_bulls_pvt_ltd/category/cabin_baggage/results/context_relevancy"
    ...
    try:
        ...
        # RAGAS Evaluation
        print(f"\n[4] Running RAGAS Context Relevancy evaluation...")
        evaluator = ContextRelevancyEvaluator(OPENAI_API_KEY)
        ...
        print("\n" + "=" * 60)
        print("RAGAS CONTEXT RELEVANCY RESULTS")
        print("=" * 60)
        ...
        for i, result in enumerate(results, 1):
            ...
            if result['success']:
                ...
                print(f"Context Relevancy Score: {score:.4f}")
        ...
        if successful_evals > 0:
            ...
            print(f"Average Context Relevancy Score: {avg_score:.4f}")

        # Find the latest serial number
        existing_files = glob.glob(os.path.join(RESULTS_DIR, "context_relevancy_results_*.xlsx"))
        max_serial = 0
        for f in existing_files:
            match = re.search(r'context_relevancy_results_(\d{1,4})\.xlsx$', f)
            if match:
        ...
        xlsx_filename = os.path.join(RESULTS_DIR, f"context_relevancy_results_{next_serial}.xlsx")
        ...
        # Update summary file
        summary_file = os.path.join(summary_dir, "context_relevancy_summary.xlsx")
